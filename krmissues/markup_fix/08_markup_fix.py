"""
Markup fixer + audit for krm.txt (KRM).
Counterpart of PWG#175, PWK#113, and siblings.

KRM-specific notes: 5 trailing in <s>, 3 trailing in <F>.

Inputs: ../../../csl-orig/v02/krm/krm.txt
        or argv[1] (any path)
...
if len(sys.argv) >= 3:
    ...
    candidates = [HERE.parent.parent.parent / "csl-orig" / "v02" / "krm" / "krm.txt", HERE / "krm.txt"]
    PW_TXT = next((p for p in candidates if p.exists()), candidates[0])
    OUT_FIX = HERE / "krm_fixed.txt"
...
AUDIT_CHECKS = [
    ...
    ("Empty content tag", re.compile(r"<(s|sup|F|Poem)\b[^>]*></\1>") if TRIM_TAGS else re.compile(r"(?!x)x")),
...
def main():
    ...
    with OUT_LOG.open("w", encoding="utf-8") as f:
        f.write(f"; markup_fix log for krm.txt\n; nested: {tot_nested}\n; whitespace: {tot_trim}\n; changed: {len(fix_log)}\n;\n")
        for ln,old,new in fix_log: f.write(f"{ln} old {old}\n{ln} new {new}\n")
    with OUT_AUDIT.open("w", encoding="utf-8") as f:
        f.write(f"KRM markup audit\n" + "="*60 + "\n\n")
        f.write(f"Generated by 08_markup_fix.py against krm.txt.\n")
        f.write(f"Paired tags: <s>, <sup>, <F>, <Poem>\n")
        f.write(f"Notes: 5 trailing in <s>, 3 trailing in <F>.\n\n")
        f.write("AUTO-FIXES: nested <ab> + whitespace in <s>, <sup>, <F>, <Poem>\n\n")
